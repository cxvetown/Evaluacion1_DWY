...
def actualizar(agregar_peluche):
    conexion = conectar()
    try:
        with conexion.cursor() as cursor:
            consulta = "UPDATE peluche_nuevo SET nombre = %s, descripcion = %s, valor=%s where id_peluche = %s;"
            #Podemos ejecutar varios query
            cursor.execute(consulta,(agregar_peluche.nombre,agregar_peluche.descripcion, agregar_peluche.valor, agregar_peluche.id_peluche))
        conexion.commit()
